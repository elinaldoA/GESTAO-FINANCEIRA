...
class InvestmentPageRenderTest extends TestCase
{
    ...
    public function test_page_renders_with_allocation_and_filter(): void
    {
        ...
        // Mounting the Resumo tab seeds the user's default investment types.
        ...
        $positions = Volt::test('investments.positions');
        $positions->assertSee('CDB Banco X')
            ->assertSee('PETR4')
            ->assertSee('% carteira');

        $positions->call('filterByType', $renda->id)
            ->assertSee('CDB Banco X')
            ->assertDontSee('PETR4');
    }
